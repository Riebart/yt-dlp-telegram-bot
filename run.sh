...
docker run --env-file ./.env -d  -v `pwd`/ollama.sock:/run/ollama.sock --restart=unless-stopped --name riebot-tg riebot-tg
docker exec --user root riebot-tg /bin/bash -c 'chmod 777 /var/run/ollama.sock; stat /var/run/ollama.sock'
